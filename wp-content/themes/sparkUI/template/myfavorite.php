...
$author_posts = new WP_Query(array('posts_per_page' => -1, 'paged' => $paged, 'orderby' => 'DESC', 'post_status' => $status,'category_name'=>'project' ));
...
<ul id="leftTab" class="nav nav-pills" style="height: 42px">
    <?php if (!hasFavorite($current_user->ID)): ?>
        <p style="margin-left: 30px;margin-top:10px;font-size: 15px"><?php _e('还没有收藏过项目.', 'frontend-publishing'); ?></p>
    <?php else: ?>
        <li class="active"><a ><?php printf(__('项目收藏（%s） ', 'frontend-publishing'), sizeof($faverite_pro)); ?></a></li>
    <?php endif; ?>
</ul>
    ...
    <div class="tab-pane fade in active" id="my-publish" style="padding-top: 40px;">
        ...
        <ul class="list-group">
            ...
            while ($author_posts->have_posts()) :$author_posts->the_post();
                $postid = get_the_ID();
                if(in_array($postid,$faverite_pro)){ ?>
...
//                    ++$count;
//                    if($count>=12){
//                        favorite_custom_pagenavi($faverite_pro,4);
//                        $count = 0;
//                    }
//                    ?>
            <?php } endwhile; ?>
