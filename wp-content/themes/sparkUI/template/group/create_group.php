...
<div class="col-md-9 col-sm-9 col-xs-12" id="col9">
    ...
    <form class="form-horizontal" role="form" name="profile" method="post" enctype="multipart/form-data"
        ...
        <div class="form-group" style="margin: 0px 0px">
            <label for="gstatus" class="col-sm-2 col-md-2 col-xs-12 control-label" style="float: left">加入方式<span
                    style="color: red">*</span></label>
            <div class="col-sm-6" style="margin-top: 7px">
                <input type="radio" id="freejoin" name="gjoin" value="freejoin" style="display: inline-block" checked/>
                <span>自由加入</span>&nbsp;&nbsp;
                <input type="radio" id="verifyjoin" name="gjoin" value="verifyjoin"
                       style="display: inline-block"/><span>检验审核加入</span>&nbsp;&nbsp;
                <input type="radio" id="verify" name="gjoin" value="verify"
                       style="display: inline-block"/><span>审核加入</span>
                <script>
                    $(function () {
                        showAddon();
                        ...
                        function showAddon() {
                            switch ($("input[name=gjoin]:checked").attr("id")) {   //根据id判断
                                case "freejoin":
                                    $("#gstatus-addon").html("<p>注:用户自由加入,无需审核</p>");
                                    break;
                                case "verifyjoin":
                                    var html = '<div style="background-color: #f2f2f2;padding-top: 10px">' +
                                        ...
                                        '</div>' +
                                        '</div>';
                                    $("#gstatus-addon").html(html);
                                    break;
                                case "verify":
                                    $("#gstatus-addon").html("<p>注:注册通过即可加入,无需填写验证信息</p>");
                                    break;
                            }
                        }
