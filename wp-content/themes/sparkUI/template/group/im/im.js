;(function (RCS) {
    ...
    var updateMessage = function (message) {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                message.targetName = response.userName;
            },
        ...
        if (newMessage.sentTime - conversation.lastSendTime >= 300000) {//超过1分钟
            var messageTime = {};
            messageTime.content = {};
                ...
                for (var i = 0; i < list.length; i++) {
                    if (list[i].conversationType == RongIMLib.ConversationType.GROUP) {
                        ...
                        jQuery.ajax({
                            ...
                            dataType: "json",
                            success: function (response) {
                                list[i].targetName = response.groupName;
                                _list.push(list[i]);
                            },
                            error: function (e, xhr, opt) {}
                        });
    ...
    var openConversation = function (conversation) {
        ...
        var callbacks = function (list, hasMsg) {
            ...
            jQuery.ajax({
                ...
                dataType: "json",
                success: function (response) {
                    data.targetName = '群组：' + response.groupName;
                },
                error: function (e, xhr, opt) {
                }
            });
    ...
    var modificateMessage = function (list) {
        ...
        for (var i = 0; i < listTemp.length; i++) {
            ...
            var ajax_data = {
                action: 'get_template_params',
                targetId: listTemp[i].senderUserId,
                targetType: 'user'
            };
            jQuery.ajax({
                async:false,   //重要!!否组ajax中的赋值不能被保存到外层
                ...
                dataType: "json",
                success: function (response) {
                    listTemp[i].senderUserName = response.userName;
                },
                error: function (e, xhr, opt) {}
            });
