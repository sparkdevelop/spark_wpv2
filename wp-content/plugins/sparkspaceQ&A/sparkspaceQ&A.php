<?php
/*
Plugin Name: sparkspaceQ&A
Plugin URI: http://www.cherie.com
Description: 此插件为火花空间的问答系统插件,目前仅用于火花空间的网站。
Version: 1.0
Author: zhangxue
Author URI: http://www.cherie.com
License: GPL
*/
if ( !class_exists( 'SS_Question_Answer' ) ) :

    class SS_Question_Answer {
        ...
        public function __construct() {
            ...
            // load posttype   所有的posttype    //对应inc中的各种类
            $this->question = new DWQA_Posts_Question();   //发布问题
            $this->answer = new DWQA_Posts_Answer();        //发表答案
            $this->comment = new DWQA_Posts_Comment();      //发表评论
            ...
            add_action( 'init', array( $this, 'init' ) );   //在wp_option数据表中添加一条记录，第二个参数为默认值
            add_action( 'widgets_init', array( $this, 'widgets_init' ) );
            add_filter( 'plugin_action_links', array( $this, 'go_pro' ), 10, 2 );   //add filter 参数1:钩子名称自己的系统中名字 参数2:回调函数(系统提供)
            add_filter( 'plugin_row_meta', array( $this, 'plugin_rows_meta' ), 10, 2 );
            register_activation_hook( __FILE__, array( $this, 'activate_hook' ) );  //注册激活时要调用的函数
            register_deactivation_hook( __FILE__, array( $this, 'deactivate_hook' ) ); //注册停用时要调用的函数
        }
        ...
        public function define_constants() {   //定义常量
            $defines = array(
                'DWQA_DIR' => plugin_dir_path( __FILE__ ),   //完整路径和路径名称
        ...
        public function widgets_init() {  //小组将初始化
            $widgets = array(
