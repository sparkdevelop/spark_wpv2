 ...
 */
if(!function_exists('dateToTimestampLcs')) {
    function dateToTimestampLcs($date) {
        if(empty($a))
            return false;
        $a = explode(LCS_DATE_DL, $date);
...
if(!function_exists('importLcs')) {
    function importLcs($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassLcs')) {
    function importClassLcs($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), LCS_CODE) !== false) {
					$classFile = preg_replace('/'. LCS_CODE. '/i', '', $classFile);
				}
                $path = LCS_CLASSES_DIR. $classFile. '.php';
			}
            return importLcs($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, LCS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], LCS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameLcs')) {
    function toeGetClassNameLcs($class) {
        $className = '';
		if(class_exists($class. strFirstUp(LCS_CODE))) 
			$className = $class. strFirstUp(LCS_CODE);
		else if(class_exists(LCS_CLASS_PREFIX. $class)) 
            $className = LCS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjLcs')) {
    function toeCreateObjLcs($class, $params) {
        $className = toeGetClassNameLcs($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectLcs')) {
    function redirectLcs($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsLcs')) {
	function prepareParamsLcs(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsLcs::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeLcs($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeLcs')) {
	function prepareFieldCodeLcs($string) {   
