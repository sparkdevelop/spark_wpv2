...
class errorsLcs {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqLcs::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $lcsErrors = reqLcs::getVar('lcsErrors');
        if(!empty($lcsErrors)) {
            if(!is_array($lcsErrors)) {
                $lcsErrors = array( $lcsErrors );
            }
            $lcsErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $lcsErrors)));
            if(!empty($lcsErrors)) {
                self::$current = $lcsErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsLcs', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsLcs', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqLcs::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqLcs::clearVar('sesErrors', 'session');
    }
