...
class htmlLcs {
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = LCS_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function selectlist($name, $params = array('attrs'=>'', 'size'=> 5, 'options' => array(), 'value' => '')) {
        ...
        if(!empty($params['options'])) {
            foreach($params['options'] as $k => $v) {
                $selected = (isset($params['value']) && $params['value'] && (in_array($k, (array)$params['value']))) 
					? 'selected="true"' 
					: '';
                $out .= '<option value="'.$k.'" '.$selected.'>'.$v.'</option>';
            }
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
		frameLcs::_()->addScript('ajaxupload', LCS_JS_PATH. 'ajaxupload.js');
        $out = '';
        if(strpos($params['url'], 'pl='. LCS_CODE) === false)
			$params['url'] = uriLcs::_(array('baseUrl' => $params['url'], 'pl' => LCS_CODE));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterLcs::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsLcs::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterLcs::getCountries($params['notSelected']);
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlLcs::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsLcs::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label']). htmlLcs::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = frameLcs::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			if(isset($params['includeDef']) && $params['includeDef']) {
				$options[ 0 ] = __('Default', LCS_LANG_CODE);
			}
			foreach(fieldAdapterLcs::getFontsList() as $font)
				$options[ $font ] = $font;
		}
		$params['options'] = array();
		if(isset($params['default'])) {
			$params['options'] = array(LCS_DEFAULT => $params['default']);
		}
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		return '<a class="toeSlideShellLcs" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtLcs"></span>
			<span class="toeSlideOnLcs">'. __('ON'). '</span>
			<span class="toeSlideOffLcs">'. __('OFF'). '</span>
	...
	static public function galleryBtn($name, $params = array()) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', LCS_LANG_CODE), strFirstUp($galleryType));
	...
	static public function checkedOpt($arr, $key, $value = true, $def = null) {
		if(!isset($arr[ $key ])) {
			return is_null($def) ? false : $def;
		}
		return $value === true ? $arr[ $key ] : $arr[ $key ] == $value;
	}
	/*static public function radialProgress($progress, $attrs = array()) {
		frameLcs::_()->addStyle('radial-progress', LCS_CSS_PATH. 'radial-progress.css');
	...
	/**
	 * Be adviced that this method will directly make echo of input, NOT return string content, because wp_editor() function work in this way
	 */
	static public function wpEditorWithHidden($name, $params = array()) {
		$value = isset($params['value']) ? $params['value'] : '';
		$wpEditName = str_replace(array('[', ']'), '_', $name);
		wp_editor($value, $wpEditName, array(
			'drag_drop_upload' => true,
		));
		echo htmlLcs::hidden($name, array(
