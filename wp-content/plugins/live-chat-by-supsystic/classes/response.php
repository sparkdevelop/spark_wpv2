...
class responseLcs {
    ...
    public function ajaxExec($forceAjax = false) {
        $isAjax = $this->isAjax();
        $redirect = reqLcs::getVar('redirect');
        if(count($this->errors) > 0)
            $this->error = true;
        if($isAjax || $forceAjax)
            exit( json_encode($this) );
        /*if($redirect)
            redirectLcs($redirect);*/
        return $this;
    }
	public function mainRedirect($redirectUrl = '') {
		$redirectUrl = empty($redirectUrl) ? LCS_SITE_URL : $redirectUrl;
		$redirectData = array();
		if(!empty($this->errors)) {
			$redirectData['lcsErrors'] = $this->errors;
		}
		if(!empty($this->messages)) {
			$redirectData['lcsMsgs'] = $this->messages;
		}
        return redirectLcs($redirectUrl. (strpos($redirectUrl, '?') ? '&' : '?'). http_build_query($redirectData));
	}
    ...
    /**
     * Alias for responseLcs::addError, @see addError method
     */
