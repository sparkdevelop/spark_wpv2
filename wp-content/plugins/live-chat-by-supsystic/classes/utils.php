...
class utilsLcs {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsLcs::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsLcs::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
	...
	static public function getWeekDaysArray( $getCurrId = false ) {
        $timestamp = strtotime('next Sunday');
		// This is made to compute days IDs - in one place in code
		// Yeh, not the est solution, but it will work in 100% - we use same IDs to identify list, and then - to identify current day ID
		if($getCurrId) {
			$currDay = strftime('%A');
		}
		$days = array();
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsLcs::getCurrentWPThemeCode(). DS; 
        }
        return $themePath;
    }
    /**
    ...
    static public function getExtModPath($plugName) {
        if(strpos($plugName, 'wp-content/themes') !== false) {  //for modules in theme dir
            return LCS_SITE_URL. str_replace(DS, '/', $plugName). '/';
        } else
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameLcs::_()->getModule($m)) 
                return false;
            if(!is_dir(frameLcs::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameLcs::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqLcs::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worlcsess post/page/custom post type content for images and return it's IDs if there are images
	...
	/**
	 * Convert RGB(A) array to string
	 */
	static public function rgbToStr($rgb) {
		return (count($rgb) == 4 ? 'rgba' : 'rgb'). '('. implode(',', $rgb). ')';
	}
