...
class fieldAdapterLcs {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterLcs', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsLcs'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsLcs::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsLcs::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameLcs::_()->getModule('products')->getCategories();
        $brands = frameLcs::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', LCS_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', LCS_LANG_CODE). '</b><br />
                '. __('Destination', LCS_LANG_CODE). ':'. htmlLcs::checkboxlist('params[dest]', array('optionsLcs' => $listOfDest)). '<br />
                '. __('Country', LCS_LANG_CODE). ':'. htmlLcs::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', LCS_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', LCS_LANG_CODE) : htmlLcs::checkboxlist('params[categories][]', array('optionsLcs' => $cOptions))). '<br />
                    <b>'. __('Brands', LCS_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', LCS_LANG_CODE) : htmlLcs::checkboxlist('params[brands][]', array('optionsLcs' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', LCS_LANG_CODE).': '. htmlLcs::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', LCS_LANG_CODE).': '. htmlLcs::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', LCS_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameLcs::_()->getModule('optionsLcs')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlLcs::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', LCS_LANG_CODE).':<br />';
                $output .= htmlLcs::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', LCS_LANG_CODE).':<br />';
                $output .= htmlLcs::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsLcs']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsLcs'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', LCS_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsLcs'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameLcs::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameLcs::_()->getTable('states')
				->leftJoin( frameLcs::_()->getTable('countries'), 'country_id' )
