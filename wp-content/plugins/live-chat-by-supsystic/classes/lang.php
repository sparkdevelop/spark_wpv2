...
class langLcs {
	...
	static public function extract($d = array('dir' => '', 'langLcs' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langLcs']) && !empty($d['langLcs']))
			$langDirPath = LCS_LANG_DIR. $d['langLcs']. DS;
		else
			$langDirPath = LCS_LANG_DIR. LCS_WPLANG. DS;
