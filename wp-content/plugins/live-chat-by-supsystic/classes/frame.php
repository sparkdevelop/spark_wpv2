...
class frameLcs {
    ...
    public function __construct() {
        $this->_res = toeCreateObjLcs('response', array());
        
    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameLcs();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsLcs::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassLcs($code. strFirstUp(LCS_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameLcs($code);
    ...
    public function init() {
		...
		dispatcherLcs::doAction('afterModulesInit');
		
		modInstallerLcs::checkActivationMessages();
        ...
        register_activation_hook(  LCS_DIR. DS. LCS_MAIN_FILE, array('utilsLcs', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(LCS_DIR. DS. LCS_MAIN_FILE, array('utilsLcs', 'deletePlugin'));
		register_deactivation_hook(LCS_DIR. DS. LCS_MAIN_FILE, array( 'utilsLcs', 'deactivatePlugin' ) );

		add_action('init', array($this, 'connectLang'));
        //$operationTime = microtime(true) - $startTime;
    }
	public function connectLang() {
		load_plugin_textdomain(LCS_LANG_CODE, false, LCS_PLUG_NAME. '/lang');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', LCS_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[LCS_METHODS]) 
                    && !empty($permissions[LCS_METHODS])
                ) {
                    foreach($permissions[LCS_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[LCS_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[LCS_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameLcs::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ LCS_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ LCS_METHODS ][ $action ]))
                            || (!is_array($permissions[ LCS_METHODS ][ $action ]) && $permissions[LCS_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[LCS_USERLEVELS])
                    && !empty($permissions[LCS_USERLEVELS])
                ) {
                    $currentUserPosition = frameLcs::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = LCS_ADMIN;
					}
                    foreach($permissions[LCS_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
                            ...
                            if(in_array($action, $lowerMethods)) {                      // Permission for this method exists
                                ...
                                if($currentUserPosition != $userlevel && $currentUserPosition != LCS_ADMIN)
                                    $res = false;
                                break;
                            }
                        } else {
    ...
    protected function _extractTables($tablesDir = LCS_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = LCS_TABLES_DIR) {
        importClassLcs('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableLcs::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameLcs::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriLcs::_($src);
		if(!$ver)
			$ver = LCS_VERSION;
    ...
    /**
     * Add all scripts from _scripts array to worlcsess
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqLcs::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqLcs::getVar('page');
		if(is_admin() && strpos($page, frameLcs::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(LCS_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(LCS_CODE. '_plugin_activation_errors');
	}
