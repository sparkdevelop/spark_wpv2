...
class dbLcs {
     ...
     * #__  Worlcsess prefix
     * ^__  Store plugin tables prefix (@see LCS_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, LCS_DB_PREF, $wpdb->prefix. LCS_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampLcs($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, LCS_DATE_DL)) return dateToTimestampLcs($date);
        $arr = explode('-', $date);
        return dateToTimestampLcs($arr[2]. LCS_DATE_DL. $arr[1]. LCS_DATE_DL. $arr[0]);
    }
	...
	static public function getCurrentTimestamp() {
		return (int) self::get('SELECT UNIX_TIMESTAMP()', 'one');
	}
}
