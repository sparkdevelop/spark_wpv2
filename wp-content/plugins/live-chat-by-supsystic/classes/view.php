<?php
abstract class viewLcs extends baseObjectLcs {
    /* 
     * @deprecated
     */
    protected $_tpl = LCS_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$code = $this->_code;
		$parentModule = frameLcs::_()->getModule( $this->_code );
		$plTemplate = frameLcs::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !frameLcs::_()->getModule($plTemplate))
			$plTemplate = '';
		if(file_exists(utilsLcs::getCurrentWPThemeDir(). 'lcs'. DS. $code. DS. $tpl. '.php')) {
            $path = utilsLcs::getCurrentWPThemeDir(). 'lcs'. DS. $code. DS. $tpl. '.php';
        } elseif($plTemplate && file_exists(frameLcs::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php')) {
			$path = frameLcs::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php';
