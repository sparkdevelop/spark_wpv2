    ...
    <div style="display: inline-block;vertical-align: top;width: 90%">
        <div style="color:gray">
            <div style="margin-bottom: 10px">
                <!--												回答者信息-->
                <a href="<?php echo site_url() . get_page_address('otherpersonal') . '&id=' . $user_id; ?>"
                ...
                <?php if (dwqa_current_user_can('edit_question', dwqa_get_question_from_answer_id())) : ?>
                    ...
                    if ($action == 'dwqa-unvote-best-answer') {
                        ?>
                        <a onclick="click_accept(<?= get_the_ID() ?>)"
                           href="<?php echo esc_url(wp_nonce_url(add_query_arg(array('answer' => get_the_ID(), 'action' => $action), admin_url('admin-ajax.php')), '_dwqa_vote_best_answer')) ?>"
                           style="float: right;color: #fe642d">
                            已采纳
                        </a>
                    <?php } else { ?>
                        <a onclick="click_accept(<?= get_the_ID() ?>)"
        ...
        <div class="answer-vote" style="margin-right: 20px;float: right;color:gray;"
             data-nonce="<?php echo wp_create_nonce('_dwqa_question_vote_nonce') ?>" data-post="<?php the_ID(); ?>">
            <a class="dwqa-vote dwqa-vote-up" href="#" onclick="click_vote(<?=$ans_id?>)"><?php _e('赞同', 'dwqa'); ?></a>
