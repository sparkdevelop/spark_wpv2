<?php
    ...
    $post_content = array();
    $answer_ids=get_children(get_the_ID($qa_id),'ARRAY_A');
    //print_r($answer_ids);
...
if (dwqa_question_answers_count($qa_id) != 0) {
    if (get_post_meta(get_the_ID($qa_id), '_dwqa_status', true) == 'open'||get_post_meta(get_the_ID($qa_id), '_dwqa_status', true) == 'answered') {?>
        <li class="list-group-item" style="padding: 15px 0px">
            <!--    获取提问者头像-->
            <div style="display: inline-block;vertical-align: top;margin-top: 0px">
                <?php echo get_avatar(get_the_author_ID($qa_id),48,'');?>
            </div>
            <div class="qa_show">
                <!--        获取提问者名字、提问时间-->
                <div class="qa_time">
                ...
                <div style="color:gray;margin-top:10px;">
                    ...
                    <div style="word-wrap: break-word; word-break: keep-all;display: inline-block;">
                        <h4>
                            ...
                            $after='</span>';
                            //if(get_the_tag_list()){
                            echo get_the_term_list( get_the_ID($qa_id), 'dwqa-question_tag', $before , $sep, $after );
                            //}
                            ?>
                        </h4>
                    </div>
    ...
    elseif (get_post_meta(get_the_ID($qa_id), '_dwqa_status', true) == 'resolved' || get_post_meta(get_the_ID($qa_id), '_dwqa_status', true) == 'close') {?>
        <li class="list-group-item" style="padding: 15px 0px">
            <div style="display: inline-block;vertical-align: top;margin-top: 0px">
                <?php echo get_avatar(get_the_author_ID($qa_id),48,'');?>
            </div>
            <div class="qa_show">
                ...
                <div style="color:gray;margin-top:10px;">
                    <div class="qa_time">
                        <?php $user_id = get_post_field( 'post_author', get_the_ID($qa_id) ) ? get_post_field( 'post_author', get_the_ID($qa_id) ) : 0;?>
