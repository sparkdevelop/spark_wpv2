...
function ludou_show_password_field()
{
    ...
    <p>
        <label for="user_school">学校（选填）<br/>
            <select class="form-control" id="user_school"  name="user_school" style="width: 100%;height: 40px;padding:3px;margin: 2px 0 16px 0 ">
    ...
    <p>
        <label for="user_sno">学号（选填）<br/>
            <input id="user_sno" class="input" type="text" size="25" value="" name="user_sno"/>
        </label>
    </p>
...
<p>
    ...
    <div class="captcha_button">
        <a id="TencentCaptcha" data-appid="2064745456" data-cbfn="callback">点击验证</a>
    </div>
...
//  $userdata['nickname'] = str_replace(array('<','>','&','"','\'','#','^','*','_','+','$','?','!'), '', $_POST['user_nick']);
    ...
    //学校填写
    $school=isset($_POST["user_school"]) ? $_POST["user_school"] : '';
    $sno = isset($_POST["user_sno"]) ? $_POST["user_sno"] : '';
    ...
    if ($school!=''){
        ...
        $sql = "INSERT INTO $wpdb->usermeta VALUES ('',$user_id,'University','$school')";
        $wpdb->get_results($sql);
        //插入角色
        $sql_ur = "INSERT INTO wp_rbac_ur VALUES ('',$user_id,$role_id,$user_id,'$modified_time')";
