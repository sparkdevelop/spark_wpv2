...
do_action('xh_social_page_wechat_qrcode_before');       
$action = apply_filters('xh_social_page_wechat_qrcode_before', null);
if(!empty($action)){
    echo $action;
    return;
}
...
switch ($login_type){
    default:
        XH_Social::instance()->WP->wp_die(XH_Social_Error::err_code(404)->errmsg,false,false);
        return;
    case 0:
        try {
            global $wpdb;
            ...
            XH_Social::instance()->WP->wp_die(XH_Social_Error::err_code(500)->errmsg,false,false);
            return;
        }
        ...
        break;
    case 1:
        require_once $addon->dir. '/includes/class-wechat-qrcode.php';
        $api =XH_Social_Channel_Wechat::instance();
        $handler = new XH_Social_Wechat_Qrcode($api->get_option('mp_id'),$api->get_option('mp_secret'));
        $code =$handler->create($wp_user_id);
        if($code instanceof XH_Social_Error){
            XH_Social::instance()->WP->wp_die($code->errmsg,false,false);
            return;
        }
        $qrcode=$code['url'];
        $uid = $code['uid'];
